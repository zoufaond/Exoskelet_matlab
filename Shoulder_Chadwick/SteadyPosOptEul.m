...
opensim_model = load('das3_eul.mat');
data = load('data_model.mat');
motion = load('mot_struct.mat');
initCond = motion.coords_struct.mot_euler_mod(1,:)';
% 
...
Aeq = [];
beq = [];
AC_bndrs = [ones(3,1)*0.01;ones(3,1)*0.01;ones(3,1)*0.005;0.005];
range = 0.15;
lb = [initCond(1:10)-AC_bndrs;fmax_vec-fmax_vec*range;lceopt_vec-lceopt_vec*range;0.5];
ub = [initCond(1:10)+AC_bndrs;fmax_vec+fmax_vec*range;lceopt_vec+lceopt_vec*range;1.5];
...
data.params.InitPosOptEul.first_elips_scale = x_new(end);
...
function [c,ceq] = moment_equilibrium(t,q,act,fmax_vec,lceopt_vec,lslack_vec,model,first_elips_scale)
    x = q(1:10);
    FO = fo_eul(t,x,zeros(10,1),model,[first_elips_scale]);
