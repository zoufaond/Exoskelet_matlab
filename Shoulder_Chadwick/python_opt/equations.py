...
def custom_objective_quat(num_coords,interval_value):
    ...
    obj_SC_wo_x = 3 * sp.Matrix([interval_value*sum((sp.Matrix(AC_pos[0:3])-sp.Matrix(x_traj[0:3])).applyfunc(lambda x: x**2))])
...
def custom_objective_eul(num_coords,interval_value):
    ...
    obj_SCrot = 3 * sp.Matrix([interval_value*sum((SCrot-sp.Matrix(x_traj[0:2])).applyfunc(lambda x: x**2))])
...
def create_eoms_eul_torqueDriven(model_struct,model_params_struct,initCond_name, derive = 'symbolic',gen_matlab_functions = None):
    # symbols
    t = sp.symbols('t')
    g,c = sp.symbols('g,c')  # 
    states = ['q','u']
