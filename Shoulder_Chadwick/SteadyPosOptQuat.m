...
opensim_model = load('das3_simplified_quat.mat');
data = load('data_model.mat');
initCond = data.params.initCond_Q;
%%
muscles = opensim_model.model_simplified_quat.muscles;
...
fun = @(x) 1e-5 * sum(mus_forces_simplified_quat(t,[zeros(4,1);x(1:qn,1);0], ...
...
Aeq = [];
beq = [];
AC_bndrs = [ones(4,1)*0.01;ones(4,1)*0.01;ones(4,1)*0.001;0.05];
lb = [initCond(1:qn)-AC_bndrs;fmax_vec-fmax_vec*0.5;lceopt_vec-lceopt_vec*0.1];
ub = [initCond(1:qn)+AC_bndrs;fmax_vec+fmax_vec*20;lceopt_vec+lceopt_vec*0.1];
% nonlcon =@(x) [0,0];
